...
#ifndef TURBOJET_TESTCASE_VIPER_OFF_DESIGN_CLASS_OFF_DESIGN_CLASS_H
#define TURBOJET_TESTCASE_VIPER_OFF_DESIGN_CLASS_OFF_DESIGN_CLASS_H
...
public:
    ...
    //Constructor
    OD();           //This may be easily replaced with ~OD() = default
    ...
    void myCustomFunction() {
        do {
            ...
            if (x == 0) {
                ...
                double *mb9 = &mbar9;
                *mb9 = ((gamma_e) / (gamma_e - 1)) *
                       pow(2 * (pow(NPR, (-2 / gamma_e)) - pow(NPR, (-1 * (gamma_e + 1)) / gamma_e)), 0.5);
            } else {
                double KH_OD = 1 - pow((r_area * rmbar), SigT);
                double T05_OD = T04_OD * (1 - KH_OD);
                double T03_OD = T02 + ((cpe / cp) * KH_OD * T04_OD);
...
#endif //TURBOJET_TESTCASE_VIPER_OFF_DESIGN_CLASS_OFF_DESIGN_CLASS_H
